...
import XcAction from './_actions';
import createPreviewHtml from './_create';

class Xc extends XcAction {
	constructor() {
		super();
				...
				extensions: [
					...
					EditorView.domEventHandlers({
						paste: e => {
							...
							if (!blob) return;
							let api = window.XcConfig.uploadAPI;
							if (!api) return;
						...
						scroll: e => {
							if (!window.XcConfig.canPreview) return;
				...
				el.on('click', e => {
					...
					switch (item.type) {
						...
						case 'about':
							...
							break;
						case 'character':
							super._createTableLists(this.cm, XcConfig.characterAPI, '星星符号', '字符大全');
							break;
						case 'emoji':
							super._createTableLists(this.cm, XcConfig.emojiAPI, '表情', '符号表情（需数据库支持）');
							break;
						case 'task-no':
						...
						case 'preview':
							...
							if (el.hasClass('active')) window.XcConfig.canPreview = true;
							else window.XcConfig.canPreview = false;
							createPreviewHtml(this.cm.state.doc.toString());
							window.XcConfig.canPreview && super._updateScroller(document.querySelector('.cm-scroller'), document.querySelector('.cm-preview'));
							break;
					}
				});
				$('.cm-tools').append(el);
	...
	/* 已测 √ */
	init_AutoSave() {
		if (window.XcConfig.autoSave !== 1) return;
...
document.addEventListener('DOMContentLoaded', () => new Xc());
