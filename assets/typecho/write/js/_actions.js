...
export default class XcAction {
		...
		this._openModal({
			...
			innerHtml: `
                <ul>
                    ...
                    <li>本编辑器仅供Xc主题使用，未经允许不得移植至其他主题！</li>
                </ul>
            `
		});
	}
	...
	handleExpression(cm) {
		$.ajax({
			url: window.XcConfig.expressionAPI,
			dataType: 'json',
			success: res => {
				let tabbarStr = '';
				let listsStr = '';
				for (let key in res) {
					...
					listsStr += `<div class="lists ${key === '泡泡' ? 'active' : ''}" data-show="${key}">${arr.map(item => `<div class="lists-item" data-text="${item.data}">${key === '颜文字' ? item.icon : `<img src="${window.XcConfig.themeURL + item.icon}">`}</div>`).join(' ')}</div>`;
				}
				this._openModal({
					title: '普通表情',
					hasFooter: false,
